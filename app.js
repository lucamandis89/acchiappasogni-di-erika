...
/* =========================
   PERSONALIZZA ACCHIAPPASOGNI (iframe configuratore.html)
   - Richiede nel DOM: #dcBack e #dcFrame (vedi index.html)
   - Bottone: #btnCustomizer
   - Il configuratore dentro iframe può chiudere inviando: parent.postMessage({type:'DC_CLOSE'}, '*')
   ========================= */
(() => {
  const back = document.getElementById('dcBack');
  const frame = document.getElementById('dcFrame');

  // Se il modal non esiste, non fare nulla (così non rompiamo l'app)
  if(!back || !frame) return;

  function openPersonalizza(){
    try{
      frame.src = 'configuratore.html?v=' + Date.now(); // anti-cache
      back.style.display = 'flex';
      document.body.classList.add('builder-open');
      document.documentElement.style.overflow = 'hidden';
    }catch(e){
      console.error(e);
      alert('Errore apertura configuratore');
    }
  }

  function closePersonalizza(){
  ...
  // Collega bottone topbar
  const btn = document.getElementById('btnCustomizer');
  if(btn){
    btn.addEventListener('click', (e)=>{
      e.preventDefault();
      openPersonalizza();
    }, true); // capture per evitare che overlay "mangi" il tap
  }

  // Fallback se nell'HTML c'è onclick="window.__openCustomizer()"
  window.__openCustomizer = openPersonalizza;

  // Chiudi cliccando sullo sfondo
  back.addEventListener('click', (e)=>{ if(e.target === back) closePersonalizza(); });

  // Chiudi con ESC
  window.addEventListener('keydown', (e)=>{ if(e.key === 'Escape' && back.style.display === 'flex') closePersonalizza(); });

  // Chiusura richiesta dal configuratore (iframe)
  window.addEventListener('message', (ev)=>{
    if(ev && ev.data && ev.data.type === 'DC_CLOSE') closePersonalizza();
  });

  // Alias per compatibilità con vecchi nomi
  window.openBuilderLite = openPersonalizza;
  window.closeBuilderLite = closePersonalizza;
})();
